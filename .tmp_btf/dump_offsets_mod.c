// 极简版: 用 kernel printk 输出偏移, 编译为 module 后 dmesg 看
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("dump_offsets");
MODULE_AUTHOR("agent");

static int __init dump_offsets_init(void)
{
#define OFF(s, f) printk(KERN_INFO "#define %-45s 0x%03lx\n", #s "_" #f "_OFF", (unsigned long)offsetof(struct s, f))
#define SIZE(s)   printk(KERN_INFO "#define SIZEOF_%s 0x%lx\n", #s, (unsigned long)sizeof(struct s))
    SIZE(task_struct);
...
static void __exit dump_offsets_exit(void) { }

module_init(dump_offsets_init);
module_exit(dump_offsets_exit);
